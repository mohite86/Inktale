...
/**
 * Attaches event listener to the window scroll event, toggling classes on the top app bar based on scroll postion.
 */
window.addEventListener('scroll', (event) => {
    // Toggling the 'active' class on the $topAppBar element based on whether the vertical scroll postion is greater than 50 pixeks
    $topAppBar.classList[window.scrollY > 50 ? 'add' : 'remove']('active');

    // Toggling the 'hide' class based on whether the current scroll postion is greater than the last scroll postion and scroll position is greater than 50 pixels
    $topAppBar.classList[window.scrollY > lastScrollPos && window.scrollY > 50 ? 'add' : 'remove']('hide');

    // Updating the last recorded scroll postion
