...
const renderSettings = async (req, res) => {
    try {
        ...
        // Log error
        console.error('Error rendering setting page: ', error.message);
        throw error;
...
/**
 * Update basic information of the logged in user such as name, username, email, bio, and profile photo.
 * 
 * @async
 * @param {Object} req - The request object
 * @param {Object} res - The response object
    ...
    try {
        ...
        if (email) {
            if (await User.exists({ email })) {
                return res.status(400).json({ message: 'Sorry, an account is already associate with this email address.' });
            }
 ...
 * @param {Object} req - The request object.
 * @param {Object} res - The response object.
 * @throws {Error} - Thorws an error if there's an issue during the process.
 */
...
module.exports = {
    renderSettings, updateBasicInfo, updatePassword, deleteAccount
}